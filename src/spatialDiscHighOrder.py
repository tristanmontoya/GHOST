...
class SpatialDiscHighOrder:
    ...
    def averagedCharacteristicLimiter2(self, u):
        ...
        for k in range(1, self.K - 1):
            ...
            #transform back
            for i in range(0,self.Np):
                u[k * self.Np * self.n_eq + i*self.n_eq : k * self.Np * self.n_eq + (i+1)*self.n_eq] = X[k,:,:] @ w_k[i*self.n_eq:(i+1)*self.n_eq]
    ...
    def LCR(self, u, k):
        ...
        C = -1.0 * self.Jinv[k] * self.D_n @ A_k + \
            self.Jinv[k] * self.Hinv_n @ (self.E_n @ A_k) + 0.5 * (
                        self.t_Rn @ (self.A(self.t_Rn.T @ u_k) - absA_roe_R) @ self.t_Rn.T) \
    ...
    def right(self, u, k):
        ...
        return -0.5 * self.Jinv[k] * self.Hinv_n @ (
                    self.t_Rn @ (self.A(self.t_Ln.T @ u_kp1) - absA_roe_R) @ self.t_Ln.T)
